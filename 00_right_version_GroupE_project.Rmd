...
library(patchwork); library(GGally); library(ggplot2); library(car); library(mgcv); library(skimr); library(viridis); library(pROC); library(caret); library(purrr); library(mgcViz);
library(psych); library(gridExtra); library(dplyr); library(MASS)
#library(ROSE) <- eventualmente lo cancelliamo
```
...
We also tried to balance the dataset, but the results were not significantly different from the unbalanced one, so we decided to keep the unbalanced dataset. This is what we expected considering that the imbalance ratio (IR) of the response variable is not too high.