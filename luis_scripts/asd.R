...
library(caret)
library(purrr)
#install.packages(c("ggplot2", "pROC", "caret", "purrr", "mgcv", "skimr", "ROSE", "mgcViz"))
#*LOAD THE DATA-----------------------------------------------------------------
...
models_assessment <- function(model, test_data, save_plots = FALSE, plot_auc_name = NULL, plot_cmatrix_name = NULL){
  ...
  res7 <- residuals(model, newdata = subset(test_data, select = -Response), type="response")
  
  br7 <- binned.resids(probabilities, res7, nclass = 50)$binned
  plot(range(br7[,1]), range(br7[,2],br7[,6], -br7[,6]), type = "n",
       xlab = "Estimated Response", ylab = "Average residual", main = "Binned Residual Plot")
