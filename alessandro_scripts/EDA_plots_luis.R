...
response_counts <- table(train_data$Response)
...
ggsave(paste0(current_path, "/PercentagesResponse.png"),
      plot = p0,
...
ggsave(paste0(current_path, "/NumericalPairs.png"),
      plot = p2,
      width = 10, height = 10, dpi = 300)

# Manually it would be something like 
# subset_data <- subset(train_data, Response == "0")
# # Calculate the density
# density_values <- density(subset_data$Age)
# plot(density_values, col = "red", main = "Density for Response = 0")


train_data$logAge <- log(train_data$Age)
train_data$logAnnual_Premium <- log(train_data$Annual_Premium)
colnames(train_data)
sum(is.null(train_data$logAnnual_Premium))
sum(is.na(train_data$logAnnual_Premium))

p3 <- ggpairs(train_data, 
...
p18 <- ggplot(train_data, aes(x = Response, y = Age, fill = Response)) +
  geom_boxplot() +
...
p19 <- ggplot(train_data, aes(x = Response, y = logAge, fill = Response)) +
  geom_boxplot() +
...
p20 <- ggplot(train_data, aes(x = Response, y = Annual_Premium, fill = Response)) +
  geom_violin() +
...
p21 <- ggplot(train_data, aes(x = Response, y = logAnnual_Premium, fill = Response)) +
  geom_violin() +
  scale_fill_manual(values = c("No" = "blue", "Yes" = "red")) +
...
ggsave(paste0(current_path, "/LogPairsAge-Annual_Premium.png"),
      plot = p3,
      width = 10, height = 10, dpi = 300) 

ggsave(paste0(current_path, "/BoxPlotAge-Annual_Premium.png"),
      plot = p22,
      width = 10, height = 10, dpi = 300)

# Create a density plot
p4 <- ggplot(train_data, aes(x = Age, fill = Response)) +
...
p5 <- ggplot(train_data, aes(x = Age, fill = Response)) +
...
# # Create a bar plot
# barplot(prop.table(table(train_data$Response, train_data$Age), margin=1),
...
#       title = "Response")
# # Create a bar plot
# barplot(prop.table(table(train_data$Response, train_data$Age), margin=2),
#         col = c("blue", "red"), 
#         xlab = "Age (years)", 
#         ylab = "Density")
# # Add a legend title
...
p7 <- ggplot(train_data, aes(x = Gender, fill = Response)) + 
...
prop.test(table(train_data$Gender, train_data$Response))
...
p8 <- ggplot(train_data, aes(x = Driving_License, fill = Response)) + 
...
px <- ggplot(train_data, aes(x = Age, fill = Response)) +
  geom_histogram() +
...
ggsave(paste0(current_path, "/Gender-Driving_License-AgeByResponse.png"),
      plot = py,
      width = 10, height = 10, dpi = 300)

# Bar plot of Gender variable by Vehicle_Damage variable
p01 <- ggplot(train_data, aes(x = Gender, fill = Vehicle_Damage)) +
...
p02 <- ggplot(train_data, aes(x = Age, fill = Vehicle_Damage)) +
  geom_density(alpha = 0.7) +
...
ggsave(paste0(current_path, "/Gender-AgeByVehicle_Damage.png"),
      plot = p1,
      width = 10, height = 10, dpi = 300)

# Bar plot of Region Code by Response in counts
# We plot a subset of labels because there are too many categories
p9 <- ggplot(train_data, aes(x = Region_Code, fill = Response)) + 
  geom_bar(position = "dodge", color = "black") +
  labs(y = "Count", fill = "Response") +
...
ggsave(paste0(current_path, "/Region_CodeByResponse.png"),
      plot = p12,
      width = 10, height = 10, dpi = 300)
...
ggsave(paste0(current_path, "/Previously_Insured-Vehicle_Age-Vehicle_DamageByResponse.png"),
      plot = p13,
      width = 10, height = 10, dpi = 300)

# Bar plot of Policy_Sales_Channel by Response in counts
# We plot a subset of labels because there are too many categories
p14 <- ggplot(train_data, aes(x = Policy_Sales_Channel, fill = Response)) + 
  geom_bar(position = "dodge", color = "black") +
  labs(y = "Count", fill = "Response") +
...
ggsave(paste0(current_path, "/PolicySalesChannelByResponse.png"),
      plot = p17,
      width = 10, height = 10, dpi = 300)

# Compute independence test of categorical variables using the Pearson's Chi-Squared test.

factor_variables <- sapply(train_data, is.factor)
