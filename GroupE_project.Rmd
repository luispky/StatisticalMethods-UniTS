...
```
From the summary of the model, we can notice that all the coefficients are statistically significant:

* For Gender there is an increment of about 9% in the interest of car insurance for male customers, with respect to females.

* For Age there is a slight decrease in the interest of the car insurance as the age of the customer grows.
...
Including the logarithm doesn't actually significantly improve the performance of the AIC.
...
```{r}
library(arm)
predicted_values <- predict(result_glm$Models$Region_Reduced, newdata = subset(unbalanced_test, select = -Response), type = "response")

y_pred <- ifelse(predicted_values >= 0.14, "Yes", "No")
residuals <- ifelse(predicted_values >= threshold, unbalanced_test$Response - predicted_values, NA)
# Calculate residuals
...
# Calculate residuals
#residuals <- residuals(result_glm$Models$Driving_License, type = "deviance")
...
# Predict probabilities
y_probs <- predict(result_glm$Models$Region_Reduced, newdata = subset(unbalanced_test, select = -Response), type = "response")

# Apply threshold to predictions
y_pred <- ifelse(y_probs >= threshold, "Yes", "No")

# Calculate residuals
residuals <- ifelse(unbalanced_test$Response == 1, unbalanced_test$Response - y_probs, unbalanced_test$Response - (1 - y_probs))

# Bin residuals based on predicted probabilities
bins <- cut(y_probs, breaks = seq(0, 1, by = 0.1), include.lowest = TRUE)
bin_means <- tapply(residuals, bins, mean)

# Plot binned residuals
plot(unique(y_probs), bin_means, type = "b", xlab = "Predicted Probability", ylab = "Mean Residual", main = "Binned Residuals")
