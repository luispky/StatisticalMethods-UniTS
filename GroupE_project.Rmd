...
###GLM summary on the AIC best models <- does it have to stay here???
...
###GLM summary on Region Reduced <- does it have to stay here???
```{r}
summary(result_glm$Models$Region_Reduced)
```

As we can see from the summary, all the variables have a p-value close to 0, hence they are all statistically significant. We can compute the exponentials of these coefficients to obtain the odds ratio: 

```{bash}
coefficienti....
(Intercept): 0.115820434067376
Previously_InsuredYes: 0.0212237185935525
Vehicle_DamageYes: 7.22774118775977
Channels_Reduced124: 0.846398044753794
Channels_Reduced152: 0.296645211621213
Channels_Reduced160: 0.108753298629103
Channels_Reduced0: 0.753240396948334
Age: 0.973935753410994
Vehicle_Age> 2 Years: 1.9270870048764
Vehicle_Age1-2 Year: 1.58835054169979
Region_Reduced28: 1.29507104853947
Region_Reduced41: 1.52836786693869
Region_Reduced46: 1.14450193557707
Region_Reduced0: 1.12261915156786
```

we can comment these results as follows: <- da modificare
...
-Compared to the baseline (intercept <- whoooo????), the odds of a customer being interested in Vehicle insurance are higher for those who live in Region 28, 41, 46 or 0. Furthermore, the odds of a customer being interested in Vehicle insurance get higher as the Age of the vehicle increases.
-Compared to the baseline (intercept <- whoooo????), the odds of a customer being interested in Vehicle insurance are lower for those being previously insured and for those having a channel of outreaching with code 124, 160, 152 or 0. Furthermore, the odds of a customer being interested in Vehicle insurance get lower as the Age of the customer increases.
...
###GAM summary on Region Reduced (2) <- does it have to stay here???
...
```{r, eval=FALSE}
exp(results_gam$Models$Region_Reduced$coefficients)
```
```{bash}
(Intercept): 0.0472232914688164
Previously_InsuredYes: 0.0212728509555799
Vehicle_DamageYes: 7.47196369221081
Channels_Reduced124: 0.825585274766525
Channels_Reduced152: 0.36644516247205
Channels_Reduced160: 0.190425915168401
Channels_Reduced0: 0.765945437210526
Region_Reduced28: 1.3033508375302
Region_Reduced41: 1.50611374045092
Region_Reduced46: 1.13160731002056
Region_Reduced0: 1.12745438452808
```

we can comment these results as follows:
-The odds of a customer being interested in Vehicle insurance are 7 times higher for those who have had a vehicle damage compared to those who didn't.
-Compared to the baseline (intercept <- whoooo????), the odds of a customer being interested in Vehicle insurance are higher for those who live in Region 28, 41, 46 or 0.
-Compared to the baseline (intercept <- whoooo????), the odds of a customer being interested in Vehicle insurance are lower for those being previously insured and for those having a channel of outreaching with code 124, 160, 152 or 0.
-Since the splines have 8.34 degrees of freedom, they are relevant for the estimation of the response variable.
...
### The best overall <- does it have to stay here???
...
print(pl, pages = 1)
```
![](images/getViz_gam_1.png)
![](images/getViz_gam_2.png)
![](images/getViz_gam_3.png)
---how do we interpret this getViz of the gam model????---
...
```{r}
#*FUNCTION TO PERFORM THE MODEL ASSESSMENT -------------------------------------
random_forest_assessment <- function(model.rf, unbalanced_test){
...
**Random Forest:** doesn't perform better than the other kind of models under any index.

It is important to consider the context of this analysis. Since we are dealing with insurance data and our target variable is whether the customer would be interested in Vehicle insurance, we may opt for the model that prioritizes the highest AUC score, as it reflects the overall ability to distinguish between customers interested and not interested in vehicle insurance, capturing both true positives and true negatives effectively. Hence, for this kind of analysis, the GAM model could be the best choice.
...
These results suggest that: <- how do we want to interpret the results of the models? do we want to comment the coefficients of each model? or do we want to comment only the one that we think is the best suitable for the problem? (like the GAM model, mentioned above)
 
* Male customers are 9% more interested in car insurance than female customers.
 
* There is a slight decrease in the odds of interest in the product with each year of increasing customer age. 
* Customers with a driver's license are 3 times more interested in the product than those without a driver's license. <- how do we interpret the intercept???
 
* There is a dramatic decrease in the interest of the customers in the product for those who previously had their cars insured. The company should focus on improving its services because current policyholders lose their interest by 98% with respect to those without car insurance.
 
* The results show that customers with older cars show more interest than customers with new cars. Around 2 times for cars more than 2 years old compared to new cars.
 
* The interest in the Car Insurance Policy of the customers with health insurance who had their car damaged in the past is more than 7 times that of those who haven´t. This is expected and a variable the company should focus on to estimate the risk associated with these customers. <- what do you mean with "to estimate the risk associated with these customers"?
 
* The amount the customer needs to pay as a premium in the year doesn't seem to be associated with any increase or decrease in the odds of the event. <- isn't it strange that the model finds this significantly relevant even though this variable doesn't affect the Response?
 
* There is more interest in the product, from ~14% to ~54% more interest, for customers from Regions 28, 41, 46, and 0 (combination of low frequent regions) with respect to Region 8. <- and the rest of the regions seems to be non relevant for the estimation..? to be checked with the meaning of the intercept
 
* The outreach Channel 26 (base category in the model) gets from 16% to 90% more interested customers in comparison with Channel 26 (what????). Channel 124 attracts a lot of clients as well and the company could attempt to increase its influence in this channel.
