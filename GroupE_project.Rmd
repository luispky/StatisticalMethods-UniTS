...
```
From the summary of the model, we can notice that all the coefficients are statistical significant:
...
```{r}
result_log <- ranking_nested_models(unbalanced_train, unbalanced_test, use_model = "glm", use_log = TRUE, use_splines = FALSE)
```


```{r}
summary(result$Models$Vehicle_Damage)
```
...
---choose best model (probably Region_Reduced) for occam razor---
...
```{r}
summary(result_log$Models$Region_Reduced)
```
...
Looking at the summary of the two models, we can see that the AIC is lower when the logarithm is not applied (idk if it's the best thing)
...
anova_values <- anova(model, test = "Chisq")
